...
from ._anvil_designer import Guides_Page1Form_copyTemplate
from anvil import *
import anvil.server
...
class Guides_Page1Form_copy(Guides_Page1Form_copyTemplate):
  def __init__(self, **properties):
    ...
    categories_motherbrd = anvil.server.call(
      "get_unique_categories", sheet_data_motherbrd
    )
    ...
    categories_storage_2 = anvil.server.call(
      "get_unique_categories", sheet_data_storage_2
    )
    self.storage_2_dropdown.items = categories_storage_2

    # this function calls from the server module and gets the sheet data for the cpus, and gets the unique categories for motherboards, then popluates the dropdown with the called items
    sheet_data_storage_3 = anvil.server.call("get_sheet_data_storage")
    categories_storage_3 = anvil.server.call(
      "get_unique_categories", sheet_data_storage_3
    )
    ...
    categories_cpu_cooler = anvil.server.call(
      "get_unique_categories", sheet_data_cpu_cooler
    )
    self.cpu_cooler_dropdown.items = categories_cpu_cooler

    # this function calls from the server module and gets the sheet data for the cpus, and gets the unique categories for motherboards, then popluates the dropdown with the called items
    sheet_data_adapters = anvil.server.call("get_sheet_data_adapters")
    categories_adapters = anvil.server.call(
      "get_unique_categories", sheet_data_adapters
    )
      ...
      if saved_builds:
        ...
        for build in saved_builds:
          ...
          btn.set_event_handler(
            "click", lambda sender=btn, **event_args: self.load_build_click(sender)
          )
  ...
  def share_button_click(self, **event_args):
    ...
    result = alert(
      content=name_form,
      title="Name Your Build for Sharing",
    ...
    if result:
      ...
      if build_name:
        ...
        app_link = anvil.server.call(
          "save_build_and_generate_link", build_name, selected_items
        )

        # Display the shareable link in an alert
        alert(
          f"Shareable Link: {app_link}\nYou can copy this link and share it with others.",
          large=True,
        )
